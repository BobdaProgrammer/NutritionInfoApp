  ...
  document.getElementById("imghold").style.display="block"
    ...
    fetch(`https://world.openfoodfacts.org/api/v2/product/${result}.json`)
      ...
      .then(data => {
        ...
        let val = 0;
        let serve = false;
        let allergens = data["product"]["allergens_from_ingredients"]
        resultEl.innerHTML = `<center><button onclick="location.reload()">Scan new item</button><h2 id="name">${
          data["product"]["product_name"]
        }</h2><h5>Last updated: ${
          data["product"]["last_edit_dates_tags"][0]
        }</h5>Add to:<br><button onclick="meal('breakfast')">Breakfast</button><button onclick="meal('lunch')">Lunch</button><button onclick="meal('dinner')">Dinner</button><button onclick="meal('snack')">Snack</button><br><input value=100 style="margin-top: 8px" class="selector" type="number" onchange="custom(event)" min=0>g</center><div id="custom"></div></center><br>Additives: ${
          data["product"]["additives_n"]
        }<br>Allergens found in ingredients: ${allergens}<br><h4>Typical values per 100g:</h4>`;
